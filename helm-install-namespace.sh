...
helm install --name echo-server-$NSNAME --namespace $NSNAME --set namespace.name=$NSNAME local/echo-server-0.1.0.tgz
check_status
...
helm install --name echo-server-ingress-$NSNAME --namespace $NSNAME --set config.test=ingress.v1 --set ingress.enabled=true --set namespace.name=$NSNAME local/echo-server-0.1.0.tgz
check_status
